...
function enviarEmail(event) {
  ...
  if (
    ...
    !alfanumericos.test(nombre.value)
  ) {
    alert(
      "El nombre completo es demasiado corto,  esta incompleto o contiene caracteres no validos"
    );
  } else {
    if (!formatoEmail.test(email.value)) {
      ...
      if (mensaje.value == " " || mensaje.value.length < 6) {
        ...
        btnMailA.setAttribute(
          "href",
          `mailto:delmontelucas678@gmail.com?subject=Nombre: ${form.get(
            "nombre"
          )}  Correo: ${form.get("email")}&body=${form.get("mensaje")}`
        );
